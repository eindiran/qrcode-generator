...
def generate_qrcode(data: str, error_correction: int = qr.constants.ERROR_CORRECT_Q,
                    box_size: int = 10, border: int = 5) -> qr.QRCode:
    '''
...
def get_error_correction_constant(desired_error_p: int) -> int:
    ...
    if desired_error_p <= 7:
        ...
        if desired_error_p > 30:
            print(f'WARNING: Specified error correction value ({desired_error_p}) is above the ' +
                  f'max supported value (30). Using max value instead.', file=sys.stderr)
...
def get_image(qrcode: qr.QRCode, factory_type: Optional[str] = None,
              fill_color: Optional[str] = None, back_color: Optional[str] = None) -> PilImage:
    '''
    '''
    ...
    if factory_type:
        ...
        if image_factory:
            print(f'WARNING: Ignoring specified colors (fill "{fill_color}" back "{back_color}") ' +
                  f'with image factory type {factory_type}: ' +
                  f'specifying both the factory type and setting the fill or back ' +
                  f'color is currently unsupported.', file=sys.stderr)
            return qrcode.make_image(image_factory=image_factory)
        else:
            print(f'WARNING: Unknown image factory type: {image_factory}. Using default.',
                  file=sys.stderr)
...
def write_image(img: PilImage, filename: str) -> None:
    ...
    try:
        ...
        print(f'ERROR: Could not write out file {filename} due to error in generating the image.',
              file=sys.stderr)
        traceback.print_exc()
    except IOError as ex:
        print(f'ERROR: Could not write out file {filename} due to IOError {ex.message}.',
              file=sys.stderr)
...
def main() -> None:
    ...
    parser.add_argument('-e', '--error-correction', dest='error_correction', type=int,
                        default=25, help='Target error correction percentage (default: 25)')
    parser.add_argument('-b', '--border-size', dest='border_size', type=int, default=5,
                        help='Size of the border in boxes (default: 5, min: 4)')
    parser.add_argument('-p', '--box-size', dest='box_size', type=int, default=10,
                        help='Pixels per box (default: 10)')
    parser.add_argument('-fc', '--fill-color', dest='fill_color', type=str, default='black',
                        help='Foreground/fill color, as HTML name or hex (default: black)')
    parser.add_argument('-bc', '--back-color', dest='back_color', type=str, default='white',
                        help='Background color, as HTML name or hex (default: white)')
    parser.add_argument('-f', '--factory', dest='factory_type', type=str,
